...
import type { CategoryItem, SoundItem, SoundMetadata, CategoryMetadata, TitleStyle, SortOrder } from "../types.js";
import { SUPPORTED_AUDIO_EXTENSIONS, SUPPORTED_IMAGE_EXTENSIONS } from "../types.js";
import { stateManager } from "./state-manager.js";

function sortItems<T extends { path: string }>(
  items: T[],
  sortOrder: SortOrder,
...
class SoundManager {
  ...
  private async scanCategoriesRecursive(folderPath: string, parentPath: string | null): Promise<CategoryItem[]> {
  ...
  private async countSubCategories(folderPath: string): Promise<number> {
    try {
      const entries = await fs.promises.readdir(folderPath, { withFileTypes: true });
  ...
  private async loadCategoryMetadata(categoryPath: string): Promise<CategoryMetadata | null> {
  ...
  async setCategoryTitle(categoryPath: string, displayName: string): Promise<void> {
    const metadataPath = path.join(categoryPath, "_config.json");
  ...
  async setSoundTitle(soundPath: string, title: string): Promise<void> {
  ...
  async setCategoryTitleStyle(categoryPath: string, titleStyle: TitleStyle | null): Promise<void> {
    const metadataPath = path.join(categoryPath, "_config.json");
  ...
  async setSoundTitleStyle(soundPath: string, titleStyle: TitleStyle | null): Promise<void> {
    const baseName = path.basename(soundPath, path.extname(soundPath));
    const metadataPath = path.join(path.dirname(soundPath), `${baseName}.json`);
  ...
  async setSoundVolume(soundPath: string, volume: number): Promise<void> {
    const baseName = path.basename(soundPath, path.extname(soundPath));
    const metadataPath = path.join(path.dirname(soundPath), `${baseName}.json`);
  ...
  private async countSounds(folderPath: string): Promise<number> {
    try {
      const entries = await fs.promises.readdir(folderPath, { withFileTypes: true });
      return entries.filter(
        (e) => e.isFile() && SUPPORTED_AUDIO_EXTENSIONS.includes(path.extname(e.name).toLowerCase())
      ).length;
    } catch {
      return 0;
    }
  ...
  private async findCategoryIcon(categoryPath: string): Promise<string | undefined> {
  ...
  private async scanSoundsInCategory(categoryPath: string): Promise<SoundItem[]> {
  ...
  async toggleFavorite(soundId: string): Promise<void> {
  ...
  async deleteSound(soundPath: string): Promise<boolean> {
    try {
      const baseName = path.basename(soundPath, path.extname(soundPath));
