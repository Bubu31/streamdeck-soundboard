...
class AudioPlayer {
    ffplayPath = "ffplay";
    async listDevices() {
        const devices = [{ id: "default", name: "Default Device" }];
        try {
            if (process.platform === "win32") {
                const result = execSync('powershell -Command "Get-AudioDevice -List | Where-Object Type -eq \\"Playback\\" | Select-Object -ExpandProperty Name"', { encoding: "utf-8", timeout: 5000 });
                ...
                const result = execSync("ffmpeg -f avfoundation -list_devices true -i \"\" 2>&1 || true", { encoding: "utf-8", timeout: 5000 });
                const matches = result.matchAll(/\[(\d+)\]\s+(.+)/g);
