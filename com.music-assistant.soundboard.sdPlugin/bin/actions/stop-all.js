...
let StopAllAction = (() => {
    ...
    var StopAllAction = class extends _classSuper {
        ...
        async updateDisplay(actionId) {
            ...
            const svg = this.generateStopIcon(hasPlaying);
            await action.setImage(svg);
            await action.setTitle(hasPlaying ? `Stop (${playingSounds.length})` : "Stop");
        }
        generateStopIcon(active) {
            const color = active ? "#d63031" : "#636e72";
            const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="144" height="144" viewBox="0 0 144 144"><rect width="144" height="144" fill="#000000"/><rect x="37" y="37" width="70" height="70" fill="${color}"/></svg>`;
