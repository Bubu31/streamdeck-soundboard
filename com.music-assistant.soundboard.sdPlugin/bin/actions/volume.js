...
let VolumeAction = (() => {
    ...
    var VolumeAction = class extends _classSuper {
        ...
        async updateDisplay(actionId) {
            ...
            const svg = this.generateVolumeIcon(settings.globalVolume, muted);
            await action.setImage(svg);
            await action.setTitle(muted ? "Muted" : `${volumePercent}%`);
        }
        generateVolumeIcon(volume, muted) {
            ...
            const barsHtml = bars
                ...
                .join("");
            const muteLine = muted ? '<line x1="25" y1="25" x2="119" y2="119" stroke="#e17055" stroke-width="6" stroke-linecap="round"/>' : "";
            const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="144" height="144" viewBox="0 0 144 144"><rect width="144" height="144" fill="#000000"/>${barsHtml}${muteLine}</svg>`;
